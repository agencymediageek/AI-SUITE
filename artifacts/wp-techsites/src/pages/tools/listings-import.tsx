...
import { DirectoryGate } from '@/components/ui/directory-gate';
import { useGetWpDashboard, getGetWpDashboardQueryKey } from '@workspace/api-client-react';
import {
  Loader2, Search, MapPin, CheckCircle2, Store,
  Coffee, Scissors, Wrench, Camera, ShoppingCart, Heart, Dumbbell,
  Car, Building2, Utensils, Pill, GraduationCap, Upload,
} from 'lucide-react';
...
export default function ListingsImportPage() {
  ...
  // Site-type detection from dashboard
  ...
  const siteType  = (dashData as any)?.site?.siteType  as string | undefined;
    ...
    <DashboardShell>
      <div className="space-y-6 animate-slide-in-up max-w-3xl">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground mb-1">Importar Listings</h1>
            <p className="text-sm text-muted-foreground">
              Coleta negócios reais via BrightData e publica automaticamente no seu diretório WordPress.
            </p>
          </div>
          <Badge className="bg-primary/10 text-primary border-primary/30 border shrink-0">
            <Store className="w-3 h-3 mr-1" /> Directory
          </Badge>
        </div>

        {/* Directory gate — shown when theme is NOT directory */}
        {siteType === 'standard' ? (
          <DirectoryGate themeName={themeName} />
        ) : (
          <>
            <ToolInfoCard
              steps={[
                { icon: '🚀', text: 'Use a Demo Rápida (1 clique) ou configure cidade + categoria manualmente' },
                { icon: '⚡', text: 'BrightData coleta nome, endereço, telefone, site e avaliações em tempo real' },
                { icon: '✅', text: 'Listings publicados automaticamente como Custom Post Type no seu WordPress' },
              ]}
              result={{ label: '🗂️ Publicado no WordPress', color: 'green', detail: '— Visível no site imediatamente como listing do diretório' }}
            />
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {imported ? (
                      <Badge className="bg-green-500/10 text-green-600 border-green-500/30 border">
                        <CheckCircle2 className="w-3 h-3 mr-1" /> Importados no WP
                      </Badge>
                    ) : (
