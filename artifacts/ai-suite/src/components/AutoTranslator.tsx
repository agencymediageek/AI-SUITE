...
export function AutoTranslator({ translations, locale, enabled }: AutoTranslatorProps) {
    ...
    const findTranslation = useCallback((text: string): string | null => {
        ...
        // Only use keys ≥ 30 chars for substring matching — short keys ("Code", "Content",
        // "More", "AI Tools" etc.) must only match as exact nodes; substring-replacing them
        // inside longer sentences creates mixed-language text.
        const sortedKeys = Array.from(new Set(
            Array.from(map.keys()).filter(k => k.length >= 30)
