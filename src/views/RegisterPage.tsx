...
// features moved inside component

interface RegisterPageProps {
    isAdmin?: boolean;
}

// formSchema moved inside component
...
function RegisterContent({ isAdmin = false }: RegisterPageProps) {
    ...
    const features: Feature[] = [
        { icon: Zap, title: t("100+ AI-powered tools") },
        { icon: Palette, title: t("Generate images & websites") },
        { icon: Code, title: t("Write code in any language") },
        { icon: Sparkles, title: t("300 free tokens to start") },
    ];
    ...
    const formSchema = z.object({
        ...
        agreeToTerms: z.boolean().refine((val) => val === true, {
            message: t("You must agree to the terms and privacy policy"),
        }),
        ...
        try {
            ...
            if (result.success) {
                if (result.requiresVerification) {
                    ...
                    toast({
                        title: `${t("Welcome to")} ${settings?.metadata?.siteName || "AI Suite"}!`,
    ...
    const handleResendVerification = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast({
                    title: t("Verification Sent"),
                    description: t("A new verification link has been sent to your email."),
                });
            } else {
                toast({
                    title: t("Error"),
                    description: data.error || t("Failed to resend link."),
                    variant: "destructive",
                });
            }
        } catch (e) {
            toast({
                title: t("Error"),
                description: t("Connection error. Please try again."),
                variant: "destructive",
            });
        } finally {
                    ...
                    <div className="space-y-8">
                        <div>
                            ...
                            <p className="text-lg text-white/80 max-w-md">
                                {t("Join thousands of creators and professionals using")} {settings?.metadata?.siteName || "AI Suite"}.
                            </p>
                        </div>
                        ...
                        <div className="p-6 bg-white/10 backdrop-blur-sm rounded-2xl">
                            <p className="text-white/90 mb-4">
                                "{settings?.metadata?.siteName || "AI Suite"} {t("transformed how I create content. What used to take hours now takes minutes.")}
                            </p>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-white/60">{t("Marketing Director")}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {requiresVerification ? (
                    ...
                    >
                        <div className="p-8 rounded-3xl border border-zinc-200 dark:border-white/5 bg-white/80 dark:bg-zinc-900/40 backdrop-blur-2xl shadow-2xl">
                            ...
                            <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-2">{t("Check Your Email")}</h2>
                            <p className="text-zinc-600 dark:text-zinc-400 text-sm mb-6 leading-relaxed">
                                {t("We sent a verification link to")} <span className="text-zinc-900 dark:text-white font-semibold">{registeredEmail}</span>. 
                                {t("Please verify your email to activate your account.")}
                            </p>
                            ...
                            {resendSuccess ? (
                                <div className="p-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-xs rounded-lg flex items-center gap-2 mb-6">
                                    ...
                                    <span>{t("A new verification link has been sent!")}</span>
                                </div>
                            ) : (
                                <Button
                    ...
                    >
                        ...
                        <div className="text-center mb-8">
                            <h2 className="text-3xl font-bold mb-2">{t("Create your account")}</h2>
