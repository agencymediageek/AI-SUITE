  ...
  const generateResume = async () => {
    if (!formData.name || !formData.position) {
      toast({
        ...
        description: t("resume.fillNamePosition"),
        variant: "destructive"
      });
      return;
    }
...
Please format it professionally with clear sections.`;
      ...
      toast({
        title: t("resume.generated"),
        description: t("resume.readyDesc")
      });
    } catch (error: any) {
      toast({
  ...
  const generateCoverLetter = async () => {
    if (!formData.name || !formData.coverLetterJob || !formData.coverLetterCompany) {
      toast({
        title: t("common.missingInfo"),
        description: t("resume.fillCoverLetter"),
        variant: "destructive"
      });
      return;
    }
...
Make it compelling and tailored to the position.`;
      ...
      toast({
        title: t("resume.coverGenerated"),
        description: t("resume.coverReadyDesc")
      });
    } catch (error: any) {
      toast({
        title: "Generation Failed",
    ...
    <div className="p-4 lg:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="text-center space-y-4">
        <div className="flex items-center justify-center gap-2">
          ...
          <h1 className="text-xl lg:text-3xl font-bold ai-gradient-text">{t("resume.title")}</h1>
        </div>
        ...
        <Card className="ai-card">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="name">{t("resume.fullName")}</Label>
                <Input
                  id="name"
                  value={formData.name}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="position">{t("resume.position")}</Label>
                <Input
                  id="position"
            ...
            <div>
              <Label htmlFor="experience">{t("resume.experience")}</Label>
              <Textarea
                id="experience"
            ...
            <div>
              <Label htmlFor="skills">{t("resume.skills")}</Label>
              <Textarea
                id="skills"
            ...
            <div>
              <Label htmlFor="education">{t("resume.education")}</Label>
              <Textarea
                id="education"
            ...
            <div className="border-t pt-4">
              <h3 className="font-semibold mb-3">{t("resume.coverLetterDetails")}</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="coverLetterJob">{t("resume.jobTitle")}</Label>
                  <Input
                    id="coverLetterJob"
                ...
                <div>
                  <Label htmlFor="coverLetterCompany">{t("resume.companyName")}</Label>
                  <Input
              ...
              {activeTab === "resume" ? (
                (isStreaming ? streamedText : resume) ? (
                  ...
                  <div className="text-center text-muted-foreground py-8">
                    {t("resume.fillInfoHint")} to create your professional resume
                  </div>
                )
              ) : (
                (isStreaming ? streamedText : coverLetter) ? (
                  ...
                  <div className="text-center text-muted-foreground py-8">
                    {t("resume.fillCoverHint")} to create your personalized letter
                  </div>
                )
              )}
            </CardContent>
