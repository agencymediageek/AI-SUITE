...
Make the questions educational, engaging, and appropriate for the ${difficulty} difficulty level.`;
      ...
      toast({
        title: "Error",
        description: error?.message || t("quiz.failedGenerate"),
        variant: "destructive"
      });
    }
  };
  ...
  if (showResults) {
    ...
    return (
      <div className="p-4 lg:p-6 space-y-6">
        <div>
          <h1 className="text-xl lg:text-3xl font-bold ai-gradient-text">{t("quiz.results")}</h1>
        ...
        <Card className="ai-card">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl">{t("quiz.finalScore")}</CardTitle>
              ...
              <CardContent>
                  ...
                  {question.options.map((option, optIndex) => (
                    ...
                    >
                      ...
                      {optIndex === question.correctAnswer && (
                        <Badge variant="default" className="ml-2">{t("quiz.correct")}</Badge>
                      )}
                    </div>
                  ))}
                </div>
                {question.explanation && (
                  <div className="mt-4 p-3 bg-ai-primary/10 border border-ai-primary/20 rounded">
                    <p className="text-sm"><strong>{t("quiz.explanation")}</strong> {question.explanation}</p>
                  </div>
                )}
              </CardContent>
            </Card>
    ...
    return (
      <div className="p-6 space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-xl lg:text-3xl font-bold ai-gradient-text">{t("quiz.inProgress")}</h1>
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-xl lg:text-3xl font-bold ai-gradient-text">{t("quiz.title")}</h1>
        <p className="text-muted-foreground mt-2">
      ...
      <Card className="ai-card">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="count">{t("quiz.questions")}</Label>
              <Select value={questionCount} onValueChange={setQuestionCount}>
                <SelectTrigger>
                  <SelectValue placeholder={t("quiz.selectCount")} />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="space-y-2">
            <Label htmlFor="custom-topic">{t("quiz.customTopic")}</Label>
            <Input
              id="custom-topic"
      ...
      {isStreaming && streamedText && (
        <Card className="ai-card">
          <CardHeader>
            <CardTitle>{t("quiz.generating")}</CardTitle>
      ...
      <Card className="ai-card">
        <CardHeader>
          <CardTitle>{t("quiz.popularSubjects")}</CardTitle>
          <CardDescription>
