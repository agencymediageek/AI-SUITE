...
export default function AdminUsers() {
    ...
    const handleResetPassword = async () => {
        ...
        try {
            ...
            if (res.ok) {
                toast({ title: t("Success"), description: t("Password reset successfully") });
    ...
    const handleDeleteUser = async (user: User) => {
        try {
            ...
            if (res.ok) {
                ...
                throw new Error(t("Failed to delete user"));
            }
        } catch (error: any) {
    ...
    const handleAddTokens = async () => {
        ...
        try {
            ...
            if (!res.ok) throw new Error(t("Failed to add tokens"));
    ...
    const handleExport = () => {
        const headers = [t("Name"), t("Email"), t("Role"), t("Status"), t("Tokens"), t("Joined")];
        const csvContent = [
            headers.join(","),
            ...users.map(user => [
                `"${user.name || ""}"`,
    ...
    const handleAddUser = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast({ title: t("Success"), description: t("User added successfully") });
            } else {
    ...
    const stats = [
        { label: t("Total Users"), value: users.length, icon: Users, color: "text-blue-500" },
        { label: t("Active"), value: users.filter((u) => u.status === "active").length, icon: CheckCircle2, color: "text-green-500" },
        { label: t("Admins"), value: users.filter((u) => u.role === "admin").length, icon: Crown, color: "text-amber-500" },
        { label: t("Disabled"), value: users.filter((u) => u.status === "disabled").length, icon: XCircle, color: "text-red-500" },
    ];
