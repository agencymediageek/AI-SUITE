...
export default function Dashboard() {
    ...
    const allowedTools = useMemo(() => {
        if (!user || user.role === "admin" || user.planName === "Enterprise" || !user.aiTools || user.aiTools.length === 0) return toolsWithRealCosts;
    ...
    const allowedQuickActions = useMemo(() => {
        if (!user || user.role === "admin" || user.planName === "Enterprise" || !user.aiTools || user.aiTools.length === 0) return quickActions;
        ...
        >
            ...
            <motion.div variants={itemVariants} className="space-y-1">
                ...
                <p className="text-muted-foreground">
                    {t("Your AI-powered workspace with")} {allowedTools.length}+ {t("tools ready to boost your productivity")}
                </p>
            </motion.div>
            ...
            <motion.div variants={itemVariants}>
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">{t("Jump right into your favorite tools")}</p>
                    </div>
                    <Button 
                        variant="ghost" 
                        size="sm" 
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-2 border-border/50 bg-card/50 backdrop-blur-sm">
                    <CardHeader className="pb-2">
                        <div className="flex items-center justify-between">
                            <div>
                                <CardTitle className="text-base">{t("Usage Overview")}</CardTitle>
                                <CardDescription>{t("Your AI generations this week")}</CardDescription>
                            </div>
                ...
                <Card className="border-border/50 bg-card/50 backdrop-blur-sm">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-base">{t("Category Usage")}</CardTitle>
                        <CardDescription>{t("Tools by category")}</CardDescription>
                    </CardHeader>
                    <CardContent>
            ...
            <motion.div variants={itemVariants}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
                    <div>
                        <h2 id="browse-all-tools" className="text-lg font-semibold scroll-m-6">Explorar Todas as Ferramentas</h2>
                        <p className="text-sm text-muted-foreground">
                            {allowedTools.length} {t("tools available")} • {newFeaturesCount} {t("new this month")}
                        </p>
                    </div>
                                ...
                                <Card className="group h-full border-border/50 bg-card/50 hover:bg-card hover:border-primary/20 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 cursor-pointer">
                                    <CardContent className="p-5">
                                        ...
                                        <p className="text-sm text-muted-foreground mb-3 line-clamp-2">
                                            {feature.description || `Gerar ${feature.label.toLowerCase()} com IA`}
                                        </p>
                    ...
                    <div className="text-center mt-6">
                        ...
                        >
                            {showAllTools ? (
                                ...
                                <>
                                    Ver Todas as {filteredFeatures.length} Ferramentas
                            ...
                            <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
                                <div className="text-center lg:text-left text-white">
                                    <div className="flex items-center gap-2 justify-center lg:justify-start mb-3">
                                        ...
                                        <span className="text-amber-300 font-semibold text-sm">Plano Pro</span>
                                    </div>
                                    <h3 className="text-2xl font-bold mb-2">
                                        Acesse Todas as {toolsWithRealCosts.length}+ Ferramentas de IA
                                    </h3>
                                    <p className="text-white/80 max-w-lg">
                                        Acesso ilimitado a todas as ferramentas, suporte prioritário e 50.000 tokens mensais.
                                    </p>
                                </div>
                                <Button
                                    size="lg"
