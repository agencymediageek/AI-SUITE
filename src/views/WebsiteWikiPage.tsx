...
export default function WebsiteWikiPage() {
    ...
    const handleExtract = async () => {
        if (!url.trim()) {
            toast({
                title: t("Invalid URL"),
                description: t("Please enter a valid website address."),
                variant: "destructive"
            });
            return;
        }
        ...
        try {
            ...
            if (!scrapeRes.ok) {
                ...
                throw new Error(errData.error || t('Scraping failed'));
            }
            ...
            if (!aiRes.ok) {
                ...
                throw new Error(errData.error || t('AI Refinement failed'));
            }

            if (!aiRes.body) throw new Error(t('Streaming not supported'));
            ...
            setTitle(url);
            toast({
                title: t("Wiki Generated!"),
                description: t("The website has been successfully converted into a Wiki using ") + selectedModel,
            });
        } catch (error: any) {
            toast({
                title: t("Process Error"),
                description: error.message || t("Failed to generate Wiki."),
                variant: "destructive"
            });
        } finally {
            setIsLoading(false);
    ...
    const copyToClipboard = () => {
        ...
        toast({
            title: t("Copied!"),
            description: t("Markdown content copied to clipboard."),
        });
    };
        ...
        <div className="flex flex-col gap-6 p-4 max-w-7xl mx-auto min-h-full">
            ...
            <header className="flex flex-col gap-2">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl lg:text-3xl font-bold tracking-tight">{t("Website Wiki")}</h1>
                        <p className="text-muted-foreground">{t("Transform any website into a structured, editable Wiki document.")}</p>
                    </div>
                </div>
            </header>
                ...
                <CardContent className="p-4 lg:p-6">
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="relative flex-1">
                            ...
                            <Input 
                                placeholder={t("Enter website URL (e.g., https://example.com/article)")} 
            ...
            <AnimatePresence mode="wait">
                {markdown ? (
                                ...
                                <div className="flex-1 flex flex-col overflow-hidden">
                                    ...
                                    <TabsContent value="edit" className="flex-1 m-0 data-[state=active]:flex data-[state=active]:flex-col">
                                        <Textarea 
                                            ...
                                            placeholder={t("Edit your wiki content here...")}
                                        />
                                    </TabsContent>
                                </div>
                            </Tabs>
                    ...
                    >
                        ...
                        <h2 className="text-xl font-bold mb-2">{t("Ready to explore?")}</h2>
                        <p className="text-muted-foreground max-w-md mx-auto mb-8">
                            {t("Enter any website URL above to start extracting knowledge. Our AI will clean, structure, and convert the page into a beautiful Wiki format.")}
                        </p>
                        <Button 
                            variant="secondary" 
